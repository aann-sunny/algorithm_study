...
현재 노드가 얼리가 아닌 경우, 연결된 친구들은 모두 얼리 어답터여야 함. 즉, 연결된 친구(자식)들이 얼리인 경우를 더해줌.
graph[node][0] += graph[friend][1];

현재 노드가 얼리인 경우, 얼리인 자식과 아닌 자식 중 최소를 더함.
graph[node][1] += Math.min(graph[friend][0], graph[friend][1]);

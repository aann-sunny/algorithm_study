# O(nm)
# 재귀를 통해 모든 원소를 방문하기 때문에 O(nm)이 된다.
# 각 원소당 for으로 4번을 반복해서 O(4nm)이 될 수 있지만 O(nm)이 된다.
...
def calc(x, y):
    if x == m-1 and y == n-1:                   # 오른쪽 맨아래(마지막지점) 근처 좌표라면
        ...
        return
    if result[x][y] != -1:                      # 계산한 적 있으면 다시 계산하지 않게.
        return
    result[x][y] = 0                            # 계산한 적 없으면 지나갔다는 표시로 0
    for dx, dy in direction:                    # 방향 확인
        nx = x+dx                               # 다음 x좌표구하기
        ny = y+dy                               # 다음 y좌표구하기
        if 0 <= nx < m and 0 <= ny < n:         # 다음 x,y좌표가 유효한지(m,n보다 큰 인덱스인지)
            if(lst[x][y] > lst[nx][ny]):        # 현재 좌표, 다음 좌표 값비교(현재보다 낮은 길만)
                calc(nx, ny)                    # 다음 좌표도 똑같이 계산

                # 갈 수 있는 방향의 점이 가진 값(마지막지점으로 갈 수 있는 길 갯수)을 더해줌.

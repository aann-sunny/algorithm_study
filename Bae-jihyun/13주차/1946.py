'''
brute force(막무가내로 풀기)인 일일히 비교하며 중첩 for문으로 찾으면
T: 20, N: 100,000
for(T):
    for(N):
        for(N):
시간복잡도 : 20 * 100,000 * 100,000 = 시간초과
...
증명!
정답 : n번째 지원자의 합격여부는 n-1 합격자에게 영향을 받는다.
가정 : n번째 지원자의 합격여부는 다른 합격자에게 영향을 받는다. (n-2 합격자가 n번째 지원자에게 영향을 준다.)
반례 : n-2번째 합격자의 점수는 n-1번째 합격자의 점수로 대체되며 target이 갱신되므로
      결국 n-2번째 합격자는 n번째 지원자에게 영향을 줄 수 없다.

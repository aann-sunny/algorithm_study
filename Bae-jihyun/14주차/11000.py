'''
접근1 : 완전탐색
S나 T가 될 수 있는 최대 10^9 크기인 배열을 만들어 그 안에 우선 0으로 강의실 갯수를 채워준다.
...
접근2 : greedy
증명!
정답 : 강의 종료시간이 제일 빠른 순서대로 다음강의를 배정해야 한다.
가정 : 다음 강의가 5(Si) 7(Ti)일때, 제일 빨리 끝나는 0 1이 있지만 제일 빨리 끝나지 않는 강의(0 4)를 선택해도 정답이다.

"0 1뒤에 5 7을 넣을 때보다 0 4 뒤에 5 7을 넣는 것이 더 손해를 보는가?"를 살펴보자.
반례 : 만약 2 3이 있는 상황에서 0 1 뒤에 5 7 을 넣으면 그 사이에 2 3을 더 넣을 수 있지만
      0 4 뒤에 5 7을 넣을 경우 2 3을 넣을 수 없다.  (= 손해)
      따라서 제일 빨리 끝나는 강의(0 1) 뒤에 강의(5 7) 을 넣어야한다.

...
만약 j가 6이면 6키로를 넣을 자리가 있는데 5키로인 물건을 가방에 넣고 싶다면, 1키로의 여유분이 남는다.
그럼 1키로일때 최대 가치인 lst[i-1][1](lst[i-1][j-w])와 현재 물건의 가치를 더한다.(5키로와 1키로의 최대가치)

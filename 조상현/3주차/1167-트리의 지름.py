...
"""
피드백 코멘트 :
처음 생각하신 아이디어도 거의 정답에 근접했네요 :) 트리의 지름에 대해 배울 수 있는 좋은 경험이 되셨으리라 생각합니다!
38번줄의 리턴값으로 start_node 를 받는다고 선언하셨는데 이 부분이 필요할까요?
작성하신 DFS 함수는 아무것도 반환하지 않고 있습니다.
게다가 만약 무언가를 반환하더라도 그 아래 41번 라인에서 -1로 덮어씌워지기 때문에 의미가 없어집니다.
"""